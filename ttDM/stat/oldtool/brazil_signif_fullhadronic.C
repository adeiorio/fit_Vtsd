...
#include "include/signif_fullhadronic_count.h"
#include "include/signif_fullhadronic_shape.h"

  unsigned int imethod = 0; if (method == "shape") imethod = 1;

#define VARRAY(name) \
  double * v_##name[2] = { y_##name##_fullhadronic_count, y_##name##_fullhadronic_shape }; \
  ...
  // The legend
  TLegend * m_legend = new TLegend(0.65,0.65,0.85,0.85);
  ...
  l.DrawLatex(0.20,0.85,"#sqrt{s} = 13 TeV, L = 1.0 fb^{-1}");
  l.SetTextSize(0.04);
  m_y_line_graph->GetXaxis()->SetRangeUser(110, 150);
...
void brazil_signif_fullhadronic() {
  system("mkdir -p plots");
  string method[] = { "count", "shape" };
  for(int i = 0; i < 2; ++i)
    brazil_signif_v(method[i], "fullhadronic");
}
