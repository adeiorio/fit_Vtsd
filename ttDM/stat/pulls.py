...
parser.add_option("--out", action="store", type="string", dest="out", default="")
...
#    content_nostat = [x for x in fullcontent if not 'CMS_stat' in x]
    ...
    b_pulls.SetFillColor(kBlue+2)
    b_pulls.SetLineColor(0)
    b_pulls.SetMarkerColor(kBlue+2)
    b_pulls.SetLineWidth(0)
    b_pulls.SetMarkerStyle(20)
    b_pulls.SetMarkerSize(1.2)

    s_pulls.SetLineColor(kRed+2)
    s_pulls.SetMarkerColor(kRed+2)
    s_pulls.SetMarkerStyle(24)
    s_pulls.SetLineWidth(2)

    rho.SetLineColor(kRed+2)
    rho.SetMarkerColor(kRed+2)
    rho.SetMarkerStyle(24)
    rho.SetLineWidth(2)

    b_pulls.GetYaxis().SetRangeUser(-6., 6.)
    s_pulls.GetYaxis().SetRangeUser(-6., 6.)
    ...
    if options.out=="":
        ...
        c1.Print(options.out+"/"+outputname+".png")
        c1.Print(options.out+"/"+outputname+".pdf")
    ...
    if options.out=="":
        ...
        c2.Print(options.out+"/"+outputname+".png")
        c2.Print(options.out+"/"+outputname+".pdf")
