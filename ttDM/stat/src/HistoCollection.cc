...
void HistoCollection::fill(const string & sampleName) {
  ...
  for(unsigned int i = 0; i < cfg.numSystematics(); ++i) {
	...
	string sys = systName;
	//	size_t found = sys.find("ST_tch");
	//       	if(found != string::npos) sys.replace(found, 6, sampleName);
       	size_t found = sys.find("ST_tch_sd");
	if(found != string::npos) sys.replace(found, 9, sampleName);
